...
alt.onServer('pet:follow', (scriptID) => {
    if (scriptID === undefined) {
        alt.log('[CLIENT] Fehler: scriptID ist undefined!');
        return;
    }

    // Warte 500 ms, bevor das Ped gesucht wird
    setTimeout(() => {
        const petEntity = native.getPedByScriptID(scriptID);
        if (!petEntity) {
            alt.log('[CLIENT] Fehler: Pet nicht gefunden!');
            return;
        }
...
function startFollowing(petEntity) {
    const petId = petEntity; // Verwende direkt die scriptID
    if (!petId) return;
